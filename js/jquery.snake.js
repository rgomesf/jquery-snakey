 ...
 * Project	: https://github.com/rgomesf/jquery-snakey
 *
 */
...
var Snake = {
        ...
        newGame: function (reset) {
            ...
            Snake.$element.find(".snake-col-map .stats-level").html(Snake.level);

            // reset lives
            Snake.lives = reset ? 3 : Snake.lives;
            Snake.$element.find(".snake-col-map .stats-lives").html(Snake.lives);

            // reset level cherries eaten and total
            Snake.$element.find(".snake-col-map .stats-eaten").html(Snake.cherriesEaten + "");
            Snake.$element.find(".snake-col-map .stats-totcherries").html(Level[Snake.level][0].cherries);

            // remove any wall & snake segments
            Snake.$element.find(".snake-col-map .wall,.snake-col-map .snake").remove();

            // hide the cherry
            Snake.$cherry.hide();

            // update map message
            Snake.$element.find(".map-msg").hide().html("Level " + Snake.level + "<small><br/>Eat <strong>" + Level[Snake.level][0].cherries + "</strong> cherries</small>").fadeIn();

            setTimeout(function () {

                // hide map message
                Snake.$element.find(".map-msg").fadeOut(500, function () {
        ...
        advance: function (val) {
            ...
            // adjust score
            Snake.score += 10;
            Snake.$element.find(".snake-col-map .stats-score").html(Snake.score);
            ...
            // adjust speed
            Snake.speed -= 1;
            Snake.$element.find(".snake-col-map .stats-speed").html(Snake.speed);
            clearInterval(Snake.animateTimer);
            Snake.animateTimer = setInterval(Snake.animate, Snake.speed);
            return false;
        },
        ...
        pause: function () {
            if (Snake.animateTimer == 0) {
                Snake.start();
                Snake.$overlay.hide();
                Snake.$element.find(".map-msg").fadeOut();
            } else {
                clearInterval(Snake.animateTimer);
                Snake.animateTimer = 0;
                Snake.$overlay.show();
                Snake.$element.find(".map-msg").html("Paused").fadeIn();
            }
        },
        ...
        gameOver: function () {
            if (Snake.lives - 1) {
                ...
                Snake.pause();
                Snake.$element.find(".map-msg").html('You Died<small><br/><a href="javascript:;" onclick="Snake.newGame(true)">Play again?</a></small>');
            }
        },

        finishedGame: function () {
            Snake.pause();
            Snake.$element.find(".map-msg").html('Well Done! You finished.<small><br/><a href="javascript:;" onclick="Snake.newGame(true)">Play again?</a></small>');
        },
    ...
    Level = [
        ,
        [
            {cherries: 5, length: 10},
            {seg: 30, top: 50, left: 120}
        ],
        [
            {cherries: 5, length: 15},
            {seg: 30, top: 50, left: 100},
            {seg: 30, top: 50, left: 200}
        ],
        [

            {cherries: 5, length: 20},
            {seg: 12, top: 10, left: 50},
            {seg: 12, top: 0, left: 250},
            {seg: 12, top: 270, left: 250},
            {seg: 12, top: 280, left: 50},
            {seg: 24, top: 80, left: 200}
        ],
        [
            {cherries: 5, length: 25},
            {seg: 19, top: 0, left: 100},
            {seg: 20, top: 200, left: 100},
            {seg: 40, top: 0, left: 0},
            {seg: 40, top: 0, left: 190}
        ],
        [
            {cherries: 5, length: 30},
            {seg: 20, top: 10, left: 50},
            {seg: 20, top: 0, left: 150},
            {seg: 20, top: 10, left: 250},
            {seg: 20, top: 0, left: 250},
        ...
        [
            ...
            {seg: 20, top: 100, left: 240},
            {seg: 20, top: 100, left: 120},
            {seg: 30, top: 50, left: 60},
            {seg: 30, top: 50, left: 140},
            {seg: 40, top: 0, left: 0},
            {seg: 40, top: 0, left: 190}
        ]
    ];
